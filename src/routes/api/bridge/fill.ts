...
export const Route=createFileRoute("/api/bridge/fill")({server:{handlers:{
  POST:async({request})=>{
    ...
    const login=String(body.login||""),platform=String(body.platform||""),commandId=String(body.commandId||"");
    ...
    const cmd=await sql.query<{id:string;status:string;login:string;platform:string}>(`select id,status,login,platform from volt_bridge_commands where id=$1`,[commandId]);
    if(!cmd.length||cmd[0].login!==login||cmd[0].platform!==platform)return Response.json({ok:false,error:"unknown_command"},{status:404});
